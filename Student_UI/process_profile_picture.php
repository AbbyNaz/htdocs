...
if(isset($_FILES['profile_picture'])){
    ...
    if (!$image_info) {
        ...
        header("Location: stud___profile.php?ERROR");
        exit;
    }

    // Update the user's profile picture in the database
    $stmt = $con->prepare("UPDATE users SET profile_picture = ? WHERE id_number = ?");
    $stmt->bind_param("si", $target_file, $_SESSION['Stud_id']);
    $stmt->execute();

    // Redirect the user back to their profile page
    header("Location: stud___profile.php?SUCCESS=".$_SESSION['Stud_id']."");
    exit;
}else{
    header("Location: stud___profile.php?NOIMAGE");
    exit;
}
